...
function showHelp() {
  ...
  $("#inst_main_body").html("<div id = 'instructions_head'>COMO JUGAR</div><p>El objetivo de Hextris es evitar que los bloques salgan del interior del hexágono gris exterior.</p><p>" + (settings.platform != 'mobile' ? 'Presiona las teclas de flecha derecha e izquierda' : 'Toca los lados izquierdo y derecho de la pantalla.') + " para girar el hexágono." + (settings.platform != 'mobile' ? ' Presiona la flecha hacia abajo para acelerar la caída del bloque.' : '') + " </p><p>Borrar bloques y obtener puntos haciendo 3 o más bloques del mismo color al tocarlos.</p>");
  if (gameState == 1) {
    pause();
  }
